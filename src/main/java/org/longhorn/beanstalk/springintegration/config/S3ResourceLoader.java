...
public class S3ResourceLoader implements ResourceLoader {
    ...
    public Resource getResource(String location) {
        try {
            ...
            throw new S3ResourceException("could not load resource from "
                    + location, e);
        }
    }
    ...
    private String getRequiredSystemProperty(String propertyName) {
        ...
        if (value == null || "".equals(value.trim())) {
            throw new S3ResourceException("no " + propertyName
                    + " property found in system");
        }
        return value;
    }
    ...
    private String getLocationPath(String location) {
        ...
        if (!resolvedLocation.startsWith(LOCATION_PREFIX)) {
            throw new S3ResourceException(resolvedLocation
                    + " does not begin with " + LOCATION_PREFIX);
        }

        return resolvedLocation.substring(LOCATION_PREFIX.length(),
                resolvedLocation.length());
